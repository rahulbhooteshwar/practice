...
const Tile = class {
    ...
    getContentOfVideo() {
        return (h("div", { class: "content-video" }, h("img", { class: "content-background-img", src: this.srcPoster, alt: dxp.i18n.t('Tile:videoPosterImageText') }), h("a", { class: "play-icon", "aria-label": dxp.i18n.t('Tile:videoPlayBtnText'), onClick: event => this.toggleVideo(true, event) }, h("img", { alt: "image", src: `${"https://asset.mastercard.com/dxp-ui/assets"}/images/icons/play-icon-${this.iconType}.png` }))));
    }
    /** show video overlay block */
    getVideoOverlay() {
        const bodyEle = document.querySelector('body');
        bodyEle.style.overflow = 'hidden';
        return (h("div", { class: "overlay-video-block" }, h("a", { role: "button", class: "btn-close", "aria-label": dxp.i18n.t('TileVideo:close'), onClick: event => this.toggleVideo(false, event) }, h("span", { class: "dxp-icon dxp-icon-large dxp-icon-close white-cross-icon" })), h("div", { class: "align-middle" }, h("dxp-video", { type: this.videoType, "icon-type": this.iconType, "src-video": this.src, "src-poster": this.srcPoster, autoplay: this.autoPlay, "disable-controls": this.disableControls, "video-name": this.videoName, "video-description": this.videoDescription, "video-upload-date": this.videoUploadDate }))));
    }
    ...
    render() {
        ...
        return (h("div", { class: `${this.base.componentClass()} ${this.isSquare ? 'square' : ''}`, dir: this.dir, "data-theme": this.theme }, styles, h("div", { class: "tile-container", tabindex: "1" }, h("div", { class: "media-container", onClick: event => !this.isSquare && this.imageClickHandler(event) }, this.backgroundType === IMAGE_BACKGROUND ? (
        ...
        h("dxp-image", { src: this.src, "src-lg": this.srcLg, "src-md": this.srcMd, responsive: this.responsive, href: this.href ? this.href : 'javascript:void(0)', "open-in-new-tab": this.openInNewTab, "focal-point": this.focalPoint }, " ")) :
            this.backgroundType === 'video-background' && this.getContentOfVideo(), h("div", { class: "cta" }, this.badgeText && h("div", { class: "badge" }, " ", h("span", { class: "badge-cont" }, h("span", { class: "badge-text" }, this.badgeText)), " "), this.eyebrowTitle && h("p", { class: "dxp-title-eyebrow dxp-font-size-sm" }, this.eyebrowTitle), this.tileCaption && h("h5", { class: "dxp-title-4" }, this.tileCaption), this.description && h("p", { class: "desc dxp-font-size-sm" }, this.description), h("slot", { name: "cta" })))), h("div", { class: "popup dxp-scrollable-container dxp-none" }, h("button", { class: "dxp-icon dxp-icon-large dxp-icon-close", "aria-label": dxp.i18n.t('Popup:closeIcon'), onClick: () => { this.closeIconClickHandler(); } }), h("div", { class: "popup-contents dxp-scrollable" }, h("slot", null))), this.isVideoShow && this.getVideoOverlay()));
    }
