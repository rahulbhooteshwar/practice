...
const Vcard = class {
    ...
    /** render card button */
    renderCardButton() {
        return (h("div", { class: "cta-wrapper", onClick: event => { event.stopPropagation(); } }, h("dxp-cta", { type: "link", "button-size": "md", "link-type": "dxp-cta-link", text: this.ctaButtonText, href: this.ctaButtonLink, "aria-label": this.ctaButtonText, disabled: !this.enableStatusText, "open-in-new-tab": "false" })));
    }
    /** render fav icon */
    renderFavIcon() {
    ...
    /** render more options  */
    renderMoreOptions() {
        return (h("div", { class: `more-options-container ${this.showMoreOption ? 'dxp-block' : ''}` }, h("div", { class: "list-wrapper", "aria-modal": "true", onClick: event => { event.stopPropagation(); } }, this.moreOptionList.length ? (h("ul", null, this.moreOptionList &&
            this.moreOptionList.map((action) => h("li", null, action.type === 'link' ?
                ...
                : '')))) : (h("slot", { name: "more-options" }))), h("button", { role: "button", class: "btn-icon btn-link close-more-options", onClick: async (event) => {
                event.stopPropagation();
