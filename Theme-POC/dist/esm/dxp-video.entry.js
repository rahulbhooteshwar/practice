...
const Video = class {
    ...
    disableVideoControls() {
        return (this.disableControls && this.customMuteButton ?
            h("button", { ref: el => this.customMuteRef = el, onClick: () => { this.toggleMute(); }, class: `${this.customMuteButtonPosition} custom-control` }, h("i", { class: `${this.muted ? 'mute-icon' : 'unmute-icon'}` }))
            : '');
    }
    /** emit analytics data */
    ...
    handleAkamai() {
        const videoposter = h("div", { ref: el => this.iframePoster = el, onClick: () => this.onPosterClick(), tabindex: "0", class: "video-thumbnail", "aria-label": dxp.i18n.t('Video:play'), style: { display: (this.autoplay) ? 'none' : 'block' } }, h("img", { "data-src": this.srcPoster, style: { display: (!this.srcPoster) ? 'none' : 'block' } }), h("div", { class: "video-poster" }, h("img", { "data-src": `${"https://asset.mastercard.com/dxp-ui/assets"}/dxp-video/play-icon-${this.iconType}.png` })));
        return (h("div", { tabindex: "0", class: this.fullHeight ? VIDEO_FULL_HEIGHT : 'embed-responsive embed-responsive-16by9' }, h("iframe", { ref: el => this.iframeRef = el, style: { display: (this.autoplay || !this.srcPoster) ? 'block' : 'none' }, class: this.fullHeight ? VIDEO_FULL_HEIGHT : 'embed-responsive-item', src: (this.autoplay || !this.srcPoster) ? this.videoSource : '', frameborder: "0", allowFullScreen: true }), videoposter));
    }
    ...
    render() {
        ...
        const video = () => {
            ...
            return (h("div", { class: this.fullHeight ? VIDEO_FULL_HEIGHT : 'embed-responsive embed-responsive-16by9' }, h("video", { ref: el => this.videoElementRef = el, class: this.fullHeight ? VIDEO_FULL_HEIGHT : 'embed-responsive-item', poster: this.srcPoster, "data-src": this.videoSource, loop: true, autoplay: this.autoplay, muted: this.muted, controls: !this.disableControls, width: this.width }), this.disableVideoControls()));
        };
        const styles = (h("span", null, h("link", { rel: "stylesheet", href: `` }), this.theme && h("link", { rel: "stylesheet", href: `` }), this.theme && h("link", { rel: "stylesheet", href: `${dxp.config.get('DXP_STYLE_BASE_URL')}/themes/${this.theme}/dxp-video.min.css` }), this.dtmUrl && (h("script", { src: this.dtmUrl }))));
