...
const Nav = class {
    ...
    async componentWillLoad() {
        ...
        if (this.apiUrl) {
            try {
                this.navData = await core$1.dxp.api(this.apiUrl);
            }
            catch (err) {
                core$1.dxp.log.error(this.element.tagName, 'componentWillLoad()', `fetch failed for ${this.apiUrl}`, err);
            }
        }
        this.utility = new overlay.CommonUtility();
        this.base.i18Init(core$1.dxp, 'Nav', messages);
        const shadow = this.element ? this.element : this.element;
        let href = ``;
        core$1.dxp.util.appendLinkElement(shadow, href);
        href = ``;
        core$1.dxp.util.appendLinkElement(shadow, href);
        href = `${core$1.dxp.config.get('DXP_STYLE_BASE_URL')}/themes/${this.theme}/dxp-nav.min.css`;
        core$1.dxp.util.appendLinkElement(shadow, href);
    }
    ...
    /** Render the nav */
    render() {
        core$1.dxp.log.debug(this.element.tagName, 'render()', `in dxp-nav render() : ${"DEVELOPMENT"}`);
        return (core$1.h("nav", { class: `${this.base.componentClass()}`, dir: this.dir, "data-theme": this.theme, role: "application" }, core$1.h("ul", { class: "nav-ul", ref: el => this.navigationContainer = el }, core$1.h("slot", null)), core$1.h("span", { role: "button", tabindex: "-1", class: `dxp-none dxp-icon dxp-icon-close`, "aria-label": core$1.dxp.i18n.t('Nav:closeIcon'), onClick: () => {
                this.closeIconClickHandler();
            } }), core$1.h("div", { class: "overlay overlay-container", onClick: () => {
                this.closeIconClickHandler();
            } }, core$1.h("span", { class: "overlay-bg-img" }), core$1.h("span", { class: "overlay-bg-opacity" }))));
    }
