...
const Comments = class {
    ...
    async componentWillLoad() {
        ...
        this.base.i18Init(core$1.dxp, 'Comments', messages.messages);
        this.config();
        if (this.apiUrl) {
    ...
    async apiService(url, queryString) {
        if (queryString && queryString !== '') {
            return core$1.dxp.api(url, queryString);
        }
        return core$1.dxp.api(url);
    }
    ...
    convertDate(date) {
        return core$1.dxp.moment(`${date.replace('T', ' ')}Z`).fromNow();
    }
    ...
    async handleApiResponse(action, text, id, replyid) {
        if (action === 'add') {
            await this.addComment(text).then(result => {
                ...
                core$1.dxp.log.info(this.element.tagName, HANDLE_API_RESPONSE, `Comment list after Add comment:`, this.domComments);
            }).catch(err => {
                core$1.dxp.log.error(this.element.tagName, HANDLE_API_RESPONSE, ERROR_OCCURRED, err);
            });
        }
        else if (action === 'edit') {
            await this.editComment(id, text).then(result => {
                ...
                if (this.apiResult) {
                    ...
                    this.commentCount = this.domComments.length;
                    core$1.dxp.log.info(this.element.tagName, HANDLE_API_RESPONSE, `Edit Comment list after edit comment:`, this.domComments);
                }
            }).catch(err => {
                core$1.dxp.log.error(this.element.tagName, HANDLE_API_RESPONSE, ERROR_OCCURRED, err);
            });
            ...
            await this.deleteComment(id).then(result => {
                ...
                if (this.apiResult) {
                    ...
                    this.commentCount = this.domComments.length;
                    core$1.dxp.log.info(this.element.tagName, HANDLE_API_RESPONSE, `Comment list after delete comment:`, this.domComments);
                }
            }).catch(err => {
                core$1.dxp.log.error(this.element.tagName, HANDLE_API_RESPONSE, ERROR_OCCURRED, err);
            });
            ...
            await this.addReply(text, id).then(result => {
                ...
                core$1.dxp.log.info(this.element.tagName, HANDLE_API_RESPONSE, `Comment list after Add reply:`, this.domComments);
            }).catch(err => {
                core$1.dxp.log.error(this.element.tagName, HANDLE_API_RESPONSE, ERROR_OCCURRED, err);
            });
        }
            ...
            await this.editReply(id, replyid, text).then(result => {
                ...
                core$1.dxp.log.info(this.element.tagName, HANDLE_API_RESPONSE, `Comment list after Edit reply:`, this.domComments);
            }).catch(err => {
                core$1.dxp.log.error(this.element.tagName, HANDLE_API_RESPONSE, ERROR_OCCURRED, err);
            });
        }
            ...
            await this.deleteReply(id, replyid).then(result => {
                ...
                if (this.apiResult) {
                    ...
                    this.commentCount = this.domComments.length;
                    core$1.dxp.log.info(this.element.tagName, HANDLE_API_RESPONSE, `Comment list after delete reply:`, this.domComments);
                }
            }).catch(err => {
                core$1.dxp.log.error(this.element.tagName, HANDLE_API_RESPONSE, ERROR_OCCURRED, err);
            });
            ...
            await this.fetchComment().then(result => {
                ...
                core$1.dxp.log.info(this.element.tagName, HANDLE_API_RESPONSE, `Fetch approved comment:`, this.domComments);
            }).catch(err => {
                core$1.dxp.log.error(this.element.tagName, HANDLE_API_RESPONSE, ERROR_OCCURRED, err);
            });
        }
    ...
    renderList(list) {
        ...
        return list.slice(0, this.lastIndex).map(item => core$1.h("dxp-comments-with-reply", { "userid-key": this.useridKey, "user-data-container": this.userDataContainer, "replies-limit": this.repliesLimit, "max-characters": this.maxCharacters, "comment-obj": JSON.stringify(item) }));
    }
    /** Render the comments */
    render() {
        core$1.dxp.log.debug(this.element.tagName, 'render()', `in dxp-comments render() : ${"DEVELOPMENT"}`);
        const styles = [
        ...
        return (core$1.h("div", { class: this.base.componentClass(), dir: this.dir, "data-theme": this.theme }, styles, core$1.h("div", { class: "comment-count" }, (this.domComments.length) ?
            core$1.h("h3", null, this.commentCount, " ", (this.commentCount > 1) ? core$1.dxp.i18n.t('Comments:multipleCommentsText') : core$1.dxp.i18n.t('Comments:singleCommentText'))
            :
                ''), core$1.h("dxp-textarea", { label: core$1.dxp.i18n.t('Comments:textarealabel'), ref: el => this.textAreaInput = el, placeholder: this.textareaPlaceholder ? this.textareaPlaceholder : core$1.dxp.i18n.t('Comments:commentplaceholder'), "max-length": this.maxCharacters, required: false }), core$1.h("div", { class: "comment-hint" }, core$1.dxp.i18n.t('Comments:commentHint', { maxlength: this.maxCharacters })), core$1.h("div", { class: "editable-block" }, core$1.h("dxp-cta-list", { "title-text": "", orientation: "horizontal" }, core$1.h("dxp-cta", { type: "button", "btn-id": "submitButton", "button-type": "primary", text: this.submitText ? this.submitText : core$1.dxp.i18n.t('Comments:submit'), onClick: e => this.submitHandler(e) }), core$1.h("dxp-cta", { type: "button", "btn-id": "cancelButton", "button-type": "secondary", text: this.cancelText ? this.cancelText : core$1.dxp.i18n.t('Comments:cancel'), onClick: e => this.submitHandler(e) }))), core$1.h("div", { class: "comment-list" }, this.domComments.length ? this.renderList(this.domComments) : '', (this.lastIndex < this.commentCount) &&
            (core$1.h("dxp-cta", { type: "button", "btn-id": "load-more-comments", "button-type": "secondary", text: core$1.dxp.i18n.t('Comments:loadmoreComments'), onClick: e => this.submitHandler(e) })))));
    }
