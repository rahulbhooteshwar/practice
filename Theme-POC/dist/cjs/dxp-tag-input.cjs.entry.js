...
const TagInput = class {
    constructor(hostRef) {
        ...
        this.tagsTempArray = [];
        this.tagItemsChanged = core.createEvent(this, "tagItemsChanged", 7);
    }
    /** actions to be performed prior to component loading */
    componentWillLoad() {
        this.base = new baseComponent_esm.BaseComponent(this, core.dxp);
        this.base.i18Init(core.dxp, 'TagInput', messages);
    ...
    clearItem(id) {
        ...
        core.dxp.log.debug(this.element.tagName, 'clearItem()', LOG_EMIT, this.tagsArray);
    }
    ...
    async getData(apiUrl, query) {
        ...
        try {
            ...
            const json = xmlApi$1.fetchRequest(apiUrl + query, opts)
                .then(data => {
                core.dxp.log.debug(this.element.tagName, 'getData()', `xhr request success`);
                return JSON.parse(data['response']);
            })
                .catch(error => {
                core.dxp.log.debug(this.element.tagName, 'getData()', `xhr request cancelled/failed : ${JSON.stringify(error)}`);
            });
            this.responseData = json;
            return this.responseData;
        }
        catch (e) {
            core.dxp.log.error(this.element.tagName, 'getData()', `fetch failed`, e);
        }
    }
    /** method to return input element reference  */
    getInputElement() {
    ...
    async handleChange(e) {
        ...
        if (query.length >= 3) {
            ...
            if (this.responseData) {
                ...
                this.suggestData = core.dxp.util.removeDuplicates(this.suggestData);
    ...
    OnBackspaceKey(target, searchBox, tags) {
        if (searchBox && this.tagsArray.length && (target.selectionStart === 0)) {
            ...
            this.tagItemsChanged.emit({ 'tagItems': this.tagsArray });
            core.dxp.log.debug(this.element.tagName, 'handleKeyUp()', LOG_EMIT, this.tagsArray);
        }
        else if (tags) {
            const id = target.id;
            this.clearItem(id);
    ...
    onListItemSelection(item) {
        ...
        this.tagItemsChanged.emit({ 'tagItems': this.tagsArray });
        core.dxp.log.debug(this.element.tagName, 'onListItemSelection()', LOG_EMIT, this.tagsArray);
    }
    ...
    onTextInput(event) {
        ...
        this.handleChange(event.target.value)
            .then(() => core.dxp.log.info(this.element.tagName, 'onTextInput()', `in handleChange`))
            .catch(err => core.dxp.log.error(this.element.tagName, 'onTextInput()', `in handleChange :${err}`));
    }
    ...
    renderSuggestionData(key) {
        return (core.h("div", null, this.suggestData[key].length ? core.h("span", { class: `title-font list-element ${LIST_CLASS}` }, key) : undefined, this.suggestData[key].map((data) => {
            return (core.h("li", { tabindex: "0", class: `list-element ${LIST_CLASS}`, "aria-label": core.dxp.i18n.t('TagInput:selectedItem', { item: data[this.keysArray[0]] }), onClick: () => this.selectSuggestion(data) }, core.h("span", null, data[this.keysArray[0]]), this.keysArray[1] && core.h("span", { class: "subtitle" }, data[this.keysArray[1]])));
        })));
    }
    ...
    render() {
        core.dxp.log.debug(this.element.tagName, 'render()', `in dxp-tag-input render() : ${"DEVELOPMENT"}`);
        if ((!this.apiUrl && !this.tagsData) || !this.keysToDisplay || !this.dataKey) {
            core.dxp.log.debug(this.element.tagName, 'render()', `component not rendered : check mandatory fields`);
            return;
        }
        const styles = [
            core.h("link", { rel: "stylesheet", href: `` }),
            [this.theme && core.h("link", { rel: "stylesheet", href: `` })],
            [this.theme && core.h("link", { rel: "stylesheet", href: `${core.dxp.config.get('DXP_STYLE_BASE_URL')}/themes/${this.theme}/dxp-tag-input.min.css` })],
            [this.theme && core.h("link", { rel: "stylesheet", href: `${core.dxp.config.get('DXP_STYLE_BASE_URL')}/themes/${this.theme}/dxp-input.min.css` })]
        ];
        const showContainer = ((this.suggestData.length || this.isSuggestData()) && this.showSearchBoxList && this.searchValue.length >= 3);
        return (core.h("div", { class: `${this.base.componentClass()} search-container `, dir: this.dir, "data-theme": this.theme }, styles, core.h("div", { class: "tag-div", onClick: () => this.focusInput() }, this.tagsArray.map(item => core.h("span", { class: "tags", tabindex: "0", id: item.id }, item[this.keysArray[0]], " ", core.h("a", { class: "dxp-icon dxp-icon-small dxp-icon-close icon-close", title: core.dxp.i18n.t('TagInput:closeText'), "aria-label": core.dxp.i18n.t('TagInput:close'), onClick: () => this.clearItem(item.id) }))), core.h("input", { type: "text", "aria-haspopup": "listbox", name: "searchbox", "aria-label": this.placeholder, id: "tagInput", class: 'dxp-form-control searchbox ', placeholder: !this.tagsArray.length && this.placeholder, onInput: event => { this.onTextInput(event); } })), core.h("div", { class: `dxp-list-container ${showContainer ? 'dxp-block' : ''}` }, core.h("ul", { class: "dxp-dropdown-list", id: "transactioncurrency", "aria-label": core.dxp.i18n.t('TagInput:listText') }, this.groupByField ?
            Object.keys(this.suggestData).map(key => {
                return this.renderSuggestionData(key);
            })
            :
                core.h("div", null, this.suggestData.map((data) => {
                    return (core.h("li", { tabindex: "0", class: `list-element ${LIST_CLASS}`, "aria-label": core.dxp.i18n.t('TagInput:selectedItem', { item: data[this.keysArray[0]] }), onClick: () => this.selectSuggestion(data) }, core.h("span", { class: "title-font" }, data[this.keysArray[0]]), this.keysArray[1] && core.h("span", { class: "subtitle" }, data[this.keysArray[1]])));
                }))))));
    }
...
exports.dxp_tag_input = TagInput;
