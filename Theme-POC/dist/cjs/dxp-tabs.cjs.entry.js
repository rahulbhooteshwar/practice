...
const viewTabsVertical = {
    render(styles) {
        return (core.h("div", { class: this.base.componentClass(), dir: this.dir, "data-theme": this.theme },
            styles,
            this.isOtherDeviceVerticalView() && this.verticalContentPosition === 'bottom' &&
                core.h("div", { class: `dxp-row vertical other-device` },
                    core.h("div", { class: "tab-wrapper-vertical dxp-col-lg-4 dxp-col-12" },
            ...
            this.isOtherDeviceVerticalView() && this.verticalContentPosition === 'accordion' &&
                core.h("div", { class: `dxp-row vertical other-device` },
                    this.renderTabsEyebrowText(),
                    this.renderTabsContent(this.tabItems),
                    core.h("div", { class: `dxp-row accordion` }, this.renderCTA())),
            !this.isOtherDeviceVerticalView() &&
                core.h("div", { class: `dxp-row vertical` },
                    core.h("div", { class: "tab-wrapper-vertical dxp-col-lg-4 dxp-col-12 dxp-col-offset-lg-1" },
                        this.renderTabsHeader(),
                        this.renderTabsEyebrowText(),
...
const Tabs = class {
    constructor(hostRef) {
        ...
        this.analyticsDataEmitter = core.createEvent(this, "dxp_comp", 7);
    }
    /** actions to be performed prior to component loading */
    componentWillLoad() {
        this.base = new baseComponent_esm.BaseComponent(this, core.dxp);
        this.base.i18Init(core.dxp, 'Tabs', messages.messages);
        this.setOrientation();
    }
    ...
    animationAddStatus(item) {
        let status = animation_utility.animator.options.none;
        if (this.animationEnabled()) {
            if (!item['active']) {
                status = animation_utility.animator.options.init;
            }
            if (item['active']) {
                status = this.animationStatus === animation_utility.animator.options.left ? animation_utility.animator.options.fadein : animation_utility.animator.options.left;
            }
            if (this.isDefaultTabElement(item)) {
                status = (this.animationStatus === animation_utility.animator.options.left || this.animationStatus === animation_utility.animator.options.fadein) ? animation_utility.animator.options.hide :
                    (!this.animationStatus ? animation_utility.animator.options.init : animation_utility.animator.options.initabsolute);
            }
        }
    ...
    animationClose(tabItemElement, callback, duration) {
        for (const item of this.nestedTabs) {
            if (tabItemElement.isEqualNode(item)) {
                item['animationStatus'] = animation_utility.animator.options.right;
            }
        }
    ...
    animationUpdateStatus(tabItemElement, reset = false) {
        ...
        if (!this.animationStatus && !this.isDefaultTabElement(tabItemElement)) {
            this.animationStatus = animation_utility.animator.options.left;
        }
    }
    /** close tab content */
    closeTabContent(e) {
        ...
        this.animationClose(tabItemElement, this.initDefaultTab, (this.animationEnabled() ? animation_utility.animator.durationReset : 0));
    }
    /** emit analytic data */
    emitAnalyticsData() {
        ...
        core.dxp.log.info(this.element.tagName, 'emitAnalyticsData()', analyticsInfoObj);
    }
    /** enable close button */
    enableCloseButton() {
    ...
    renderArrow(arrowOrientation) {
        return (core.h("span", { class: "arrow-container" }, core.h("span", { class: `arrow ${arrowOrientation}` })));
    }
    /** render CTA */
    renderCTA() {
        return (this.enableCta ?
            (this.cta ?
                core.h("dxp-cta", { "button-type": this.cta.linkType, "icon-align": this.cta.iconAlign, text: this.cta.text, type: this.cta.type, src: this.cta.src, "aria-label": this.cta.ariaLabel, href: this.cta.href })
                :
                    core.h("slot", { name: "cta-list" })) : '');
    }
    /** render tab control */
    renderTabControl(tabItemElement, arrowOrientation) {
        return (core.h("div", { class: `tab-control ${tabItemElement['active'] ? 'tab-control-active' : ''} ${this.fixedWidth ? 'tab-fixed-width' : ''}`, onClick: () => this.activateTab(tabItemElement, false, true) }, core.h("div", { tabindex: `${tabItemElement['active'] ? 0 : -1}`, role: "button", class: "tab-title", onFocus: event => this.focusedControl = event.target }, this.renderTabIcon(tabItemElement), this.renderTabTitleText(tabItemElement), this.renderArrow(arrowOrientation))));
    }
    /** render tab icon */
    renderTabIcon(tabItem) {
        ...
        return (core.h("div", { class: "tab-icon-wrapper" }, core.h("img", { src: tabItem['tabIconSrc'], alt: tabItem['alt'] ? tabItem['alt'] : tabItem['tabTitle'] })));
    }
    /** render tab items */
    renderTabItems(tabItems) {
        return (core.h("div", { class: "tab-items-container" }, tabItems
            ? tabItems.map(tabItem => core.h("dxp-tab-item", { active: tabItem['active'], "is-default": tabItem['isDefault'], "is-default-view": tabItem['isDefaultView'], "is-default-view-on": this.base.returnBooleanValue(this.getDefaultTabElement()), "vertical-align": this.verticalAlign, "enable-close": this.enableCloseButton(), "enable-animation": this.animationEnabled(), "animation-status": tabItem['animationStatus'], "tab-title": tabItem['tabTitle'], content: tabItem['content'], "tab-icon-src": tabItem['tabIconSrc'], alt: tabItem['alt'], "vertical-content-position": this.verticalContentPosition }))
            : core.h("slot", null)));
    }
    /** render tabs control */
    renderTabs(tabItems, arrowOrientation = '') {
        return (core.h("div", { class: `tab-control-group ${this.verticalAlign ? '' : 'dxp-col-12'} ${this.verticalAlign ? 'tab-vertical' : ''} ${this.verticalAlign ? 'dxp-col-12' : ''}` }, tabItems.filter(item => !this.isDefaultTabElement(item)).map(tabItemElement => this.renderTabControl(tabItemElement, arrowOrientation))));
    }
    /** render tabs content */
    renderTabsContent(tabItems) {
        return (core.h("div", { class: `item-wrapper ${this.getDefaultTabElement() ? 'default-view-on' : 'default-view-off'}
       ${this.verticalAlign ? 'item-wrapper-vertical dxp-col-lg-6 dxp-col-offset-lg-1 default-view-container' : ''}
        ${this.isDefaultTabActive ? 'default-tab-active' : ''}` }, core.h("div", { class: `${this.verticalAlign ? 'item-content' : 'item-container'}` }, this.renderTabItems(tabItems))));
    }
    ...
    renderTabsHeader() {
        ...
        return (core.h("div", { class: "tabs-header-container tabs-title dxp-col-12" }, this.tabsTitle &&
            core.h("h1", { tabindex: "0" }, this.tabsTitle), this.tabsTitle && this.tabsDescription &&
            core.h("div", { class: "seperator" }, "\u00A0"), this.tabsDescription &&
            core.h("div", { tabindex: "0", class: "tabs-description" }, this.tabsDescription)));
    }
    /** render tabs title */
    renderTabTitleText(tabItem) {
        return (core.h("span", { class: `tab-title-text ${this.iconOnlySm && !this.verticalAlign ? 'icon-only-sm' : ''}`, innerHTML: tabItem['tabTitle'] }));
    }
    /** reset wrapper */
    resetVContainer() {
    ...
    render() {
        const styles = [
            ...
            [this.theme && core.h("link", { rel: "stylesheet", href: `${core.dxp.config.get('DXP_STYLE_BASE_URL')}/themes/${this.theme}/dxp-tabs.min.css` })]
        ];
        if (this.verticalAlign) {
            core.dxp.log.debug(this.element.tagName, 'render()', `call in dxp-tabs-vertical render() : ${"DEVELOPMENT"}`);
            return viewTabsVertical.render.call(this, styles);
        }
        core.dxp.log.debug(this.element.tagName, 'render()', `in dxp-tabs render() : ${"DEVELOPMENT"}`);
        return (core.h("div", { class: this.base.componentClass(), dir: this.dir, "data-theme": this.theme }, styles, core.h("div", { class: "dxp-row horizontal" }, this.renderTabsHeader(), this.renderTabsEyebrowText(), this.renderTabs(this.nestedTabs), this.renderTabsContent(this.tabItems))));
    } // render
