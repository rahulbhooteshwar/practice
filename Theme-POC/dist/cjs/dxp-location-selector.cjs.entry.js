...
const LocationSelector = class {
    constructor(hostRef) {
        ...
        this.searchLocationChanged = core$1.createEvent(this, "searchLocationChanged", 7);
    }
    /** actions to be performed prior to component loading */
    componentWillLoad() {
        this.base = new baseComponent_esm.BaseComponent(this, core$1.dxp);
        this.base.i18Init(core$1.dxp, 'LocationSelector', messages);
    }
        ...
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(position => {
                this.getLocationCity(position.coords.latitude, position.coords.longitude).then(val => {
                    ...
                    .catch(error => {
                    core$1.dxp.log.debug(this.element.tagName, 'getLocation()', `current location location notfound : ${JSON.stringify(error)}`);
                });
            });
        }
        else {
            core$1.dxp.log.debug(this.element.tagName, 'getLocation()', `Sorry, your browser does not support HTML5 geolocation`);
        }
    }
    ...
    async getLocationCity(latitude, longitude) {
        ...
        const filterItemsJson = await core$1.dxp.api(apiCurrentLocation);
    ...
    onTextInput(event) {
        ...
        if (this.searchValue.trim().length >= 1) {
            ...
            const apiEndpoint = `${"https://maps.googleapis.com/maps/api/place/autocomplete/json?key="}${this.apiKey}&types=(${this.searchType})&language=${core$1.dxp.i18n.languages[0]}&input=${this.searchValue}`;
    ...
    renderLocationValues() {
        return (core$1.h("div", { class: `dxp-location-list-container  ${this.filterItemsJson && this.filterItemsJson.length ? ' with-result ' : ''}
       ${(this.showSearchBoxList) ? ' dxp-block' : 'dxp-none'}` }, core$1.h("ul", { class: `dxp-suggestion-list ${!this.searchValue.length ? 'current-location' : ''}`, "aria-label": core$1.dxp.i18n.t('LocationSelector:listText') }, (this.filterItemsJson && this.filterItemsJson.length) ? this.filterItemsJson.map(suggestTerm => ([
            core$1.h("li", { tabIndex: -1, class: "dxp-dropdown-list-item" }, !this.searchValue.length && !this.responseFlag && core$1.h("p", { class: "dxp-font-size-sm dxp-pb-3" }, " Your location"), core$1.h("p", { class: "current-location-data", onClick: this.selectSuggestion.bind(this) }, !this.searchValue.length && !this.responseFlag && core$1.h("span", { class: "dxp-icon dxp-icon-current-location", tabIndex: 0, "aria-label": core$1.dxp.i18n.t('LocationSelector:searchIconAccessibilityText') }), suggestTerm.description)),
            !this.searchValue.length && !this.responseFlag && core$1.h("li", { class: "dxp-dropdown-list-item" }, "All location")
        ]))
            :
                core$1.h("li", { tabIndex: -1, class: "dxp-dropdown-list-item no-result-found", "aria-label": core$1.dxp.i18n.t('LocationSelector:noResultFound') }, core$1.h("p", null, core$1.dxp.i18n.t('LocationSelector:noResultFound'))))));
    }
    ...
    render() {
        core$1.dxp.log.debug(this.element.tagName, 'render()', `in dxp-location-selector render() : ${"DEVELOPMENT"}`);
        const styles = [
            core$1.h("link", { rel: "stylesheet", href: `` }),
            [this.theme && core$1.h("link", { rel: "stylesheet", href: `` })],
            [this.theme && core$1.h("link", { rel: "stylesheet", href: `${core$1.dxp.config.get('DXP_STYLE_BASE_URL')}/themes/${this.theme}/dxp-location-selector.min.css` })]
        ];
        return (core$1.h("div", { role: "application", class: `${this.base.componentClass()} search-wrapper simple-search search-box-container`, dir: this.dir, "data-theme": this.theme }, styles, core$1.h("div", { class: "search-box-container" }, core$1.h("div", { class: "wrapper" }, core$1.h("input", { type: "search", "aria-haspopup": "listbox", name: "searchbox", "aria-label": core$1.dxp.i18n.t('LocationSelector:customMessage'), class: `dxp-form-control searchbox ${(this.showSearchBoxList && this.filterItemsJson && this.filterItemsJson.length) ? 'dxp-border-radius' : ''}`, placeholder: `${this.searchLocationData ? this.searchLocationData[0].placeholder : 'All Location'}`, onInput: event => { this.onTextInput(event); }, onClick: () => this.toggleCurrentLocation() }), core$1.h("span", { class: "dxp-icon dxp-icon-sm dxp-icon-globe search-text", tabIndex: 0, "aria-label": core$1.dxp.i18n.t('LocationSelector:searchIconAccessibilityText') }), !this.searchValue.length && core$1.h("span", { class: `dxp-icon dxp-icon-sm ${this.showDownArrow ? 'dxp-icon-caret-down' : 'dxp-icon-caret-up'}`, tabIndex: 0, "aria-label": core$1.dxp.i18n.t('LocationSelector:searchIconAccessibilityText'), onClick: () => this.toggleCurrentLocation() }), core$1.h("span", { class: `dxp-icon dxp-icon-sm dxp-icon-close dxp-icon-clear ${this.filterItemsJson && this.searchValue.length >= 1 ? ' dxp-block' : ' dxp-none'}`, "aria-label": core$1.dxp.i18n.t('LocationSelector:clear'), tabIndex: 0, onClick: () => this.clearSearchBox() })), this.renderLocationValues())));
    }
...
exports.dxp_location_selector = LocationSelector;
