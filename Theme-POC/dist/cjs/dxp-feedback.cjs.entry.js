...
const Feedback = class {
                ...
                if (bodyData.ratings[0]) {
                    Number.isInteger(bodyData.ratings[0]['ratingValue']) ?
                        this.feedbackListDisplay.unshift({
                            ...
                            timeStamp: core$1.dxp.moment(new Date()).fromNow(),
        ...
        if (this.apiUrl && (this.feedbackItemList.length === 1)) {
            ...
            if (apiResultFeedbackList && apiResultFeedbackList.length > 0) {
                feedbackList = [...apiResultFeedbackList].reverse().filter((feedback) => feedback.ratings[0]).map((feedback) => Number.isInteger(feedback.ratings[0]['ratingValue']) ? ({
                    ...
                    timeStamp: core$1.dxp.moment(`${feedback.createdDate.replace('T', ' ')}Z`).fromNow(),
        ...
        if (this.countDisplayElement) {
            this.countDisplayElement.innerHTML = this.feedbackListDisplay.length > 0 ?
                ` ${this.feedbackListDisplay.length} <span class="feedback-heading">${(this.feedbackListDisplay.length > 1) ?
                    core$1.dxp.i18n.t('Feedback:Reviews') : core$1.dxp.i18n.t('Feedback:Review')}</span>` : '';
        }
        if (this.loadMoreElement) {
    ...
    showAverageDisplayElement() {
        ...
        <div class='render-stars' role='img' aria-label= ${core$1.dxp.i18n.t('Feedback:star')}>  ${this.renderStars()} </div>
        ${core$1.dxp.i18n.t('Feedback:basedOn')} ${this.feedbackListDisplay.length} ${(this.feedbackListDisplay.length > 1) ?
                core$1.dxp.i18n.t('Feedback:ratings') : core$1.dxp.i18n.t('Feedback:rating')}.` : '';
        }
    }
    ...
    render() {
        const styles = [
            ...
            [this.theme && (core$1.h("link", { rel: "stylesheet", href: `${core$1.dxp.config.get('DXP_STYLE_BASE_URL')}/themes/${this.theme}/dxp-feedback.min.css` }))]
        ];
        return (core$1.h("div", { class: `container ${this.base.componentClass()}`, dir: this.dir, "data-theme": this.theme }, styles, core$1.h("h3", { class: "feedback-title-display" }, this.feedbackTitle), core$1.h("div", { class: "feedback-item-wrapper" }, core$1.h("div", { class: "feedback-items", ref: el => (this.itemContainer = el) }, core$1.h("slot", null)), this.isAdditionalCommentRequired &&
            core$1.h("div", null, this.feedbackAdditionalText, core$1.h("div", { class: "textarea-wrapper" }, core$1.h("dxp-textarea", { rows: 3, "max-length": 500, placeholder: this.feedbackAdditionalTextPlaceholder, onInput: e => this.handleTextarea(e), ref: el => this.textAreaElement = el }))), core$1.h("div", { class: "btn-wrapper" }, core$1.h("div", { class: "submit-btn" }, core$1.h("dxp-cta-list", null, core$1.h("dxp-cta", { type: "button", "btn-id": "submit", "button-type": "primary", text: "Submit", disabled: this.isSubmitDisabled, onClick: e => this.submitHandler(e) }), core$1.h("dxp-cta", { type: "button", "btn-id": "cancel", "button-type": "secondary", text: "Cancel", onClick: e => this.submitHandler(e) }))))), this.feedbackItemList.length === 1 ?
            core$1.h("div", { class: "display-wrapper" }, this.feedbackItemList[0][DISPLAY_AS] === 'star' ? core$1.h("div", { class: "avg-rating-wrapper" }, core$1.h("div", { class: "average-rating", ref: el => this.averageDisplayElement = el }), core$1.h("div", { class: "star-rating", ref: el => this.starAverageDisplay = el }, this.avgStars.map(avgStar => {
                return (core$1.h("div", { class: "star-line" }, core$1.h("span", null, avgStar.star, core$1.h("i", { class: "active", role: "img", "aria-label": `${core$1.dxp.i18n.t('Feedback:starRatedBy')} ${avgStar.rating} ${core$1.dxp.i18n.t('Feedback:percentageUsers')}` })), core$1.h("div", { class: "grey-line" }, core$1.h("div", { class: "average-line" }))));
            }))) : '', this.feedbackItemList[0][DISPLAY_AS] !== 'star' ? core$1.h("h3", { ref: el => this.countDisplayElement = el }) : '', core$1.h("div", { class: "feedback-display", ref: el => (this.itemContainerDisplay = el) }), core$1.h("div", { class: "load-more", ref: el => (this.loadMoreElement = el) }, core$1.h("dxp-cta", { type: "button", "btn-id": "load-more", "button-type": "secondary", text: core$1.dxp.i18n.t('Feedback:showMoreReview'), onClick: e => this.submitHandler(e) }))) : ''));
    }
