...
const SelectorMenu = class {
    constructor(hostRef) {
        ...
        this.value = '';
        this.apiCallCompleted = core$1.createEvent(this, "apiCallCompleted", 7);
        this.clearValue = core$1.createEvent(this, "clearValue", 7);
        this.searchEmitter = core$1.createEvent(this, "searchEmitter", 7);
        this.selectedItemChanged = core$1.createEvent(this, "selectedItemChanged", 7);
        this.toggleEmitter = core$1.createEvent(this, "toggleEmitter", 7);
    }
    /** actions to be performed prior to component loading */
    componentWillLoad() {
        this.base = new baseComponent_esm.BaseComponent(this, core$1.dxp);
        this.base.i18Init(core$1.dxp, 'SelectorMenu', messages);
    ...
    renderDefaultSelector() {
        return (core$1.h("div", { class: "selector-button-wrapper" }, core$1.h("button", { tabindex: `${this.disabled ? -1 : 0}`, id: this.selectorId, title: this.value, disabled: this.disabled, class: `default-selector selector-button dxp-text-truncate ${this.disabled ? DISABLED_CLASS : ''} ${this.isValid ? '' : HAS_ERROR}
        ${this.value ? 'more-padding' : 'has-placeholder'}`, onClick: () => this.toggleComponent() }, !this.value ?
            this.placeholder
            : this.value, core$1.h("span", { class: `select-caret ${this.toggle ? ROTATE_ICON : ''} ${this.disabled ? DISABLED_CLASS : ''}` }, core$1.h("i", { class: `${this.iconSprite} caret-down-sm-b` }))), core$1.h("button", { role: "button", tabindex: `${this.disabled ? -1 : 0}`, class: `clear-search-btn ${!this.value ? 'dxp-none' : ''} ${this.disabled ? DISABLED_CLASS : ''}`, onClick: event => this.clearSelectorMenu(event), "aria-label": "Clear selected value" }, core$1.h("i", { class: "dxp-icon dxp-icon-close" }))));
    }
    /** render list item */
    renderItem(item) {
        if (item.status) {
            return (core$1.h("div", { class: "has-icon" }, this.renderListItem(item), this.renderListItemIcon(item.status)));
        }
        return (this.renderListItem(item));
    }
    /** render label */
    renderLabel() {
        return (core$1.h("label", { class: `dxp-input-label ${this.disabled ? 'dxp-disabled' : ''} ${!this.isValid && !this.isOptional ? 'dxp-error' : ''}` }, this.label, " ", this.isOptional ? core$1.h("span", { class: "dxp-optional" }, " (optional)") : ''));
    }
    /** Render Link Selector menu */
    renderLinkSelector() {
        return (core$1.h("button", { role: "button", tabindex: `${this.disabled ? -1 : 0}`, id: this.selectorId, class: `btn-link selector-button ff-medium dxp-text-truncate ${(this.disabled ? DISABLED_CLASS : '')}`, onClick: () => this.toggleComponent() }, !this.value ?
            this.placeholder
            : this.value, core$1.h("span", { class: `select-caret ${this.toggle ? ROTATE_ICON : ''} ${this.disabled ? DISABLED_CLASS : ''}` }, core$1.h("i", { class: `${this.iconSprite} arrow-down-xs-o` }))));
    }
    ...
    renderListItemIcon(iconStatus) {
        ...
        return core$1.h("span", { class: 'icon-wrapper' }, core$1.h("i", { class: `${this.iconSprite} ${iconStatus}` }));
    }
    /** Render Detaile Selector menu */
    renderOptionWithDetailsSelector() {
        return (core$1.h("button", { role: "button", id: this.selectorId, tabindex: `${this.disabled ? -1 : 0}`, class: `option-with-details-button selector-button ${(this.disabled ? DISABLED_CLASS : '')}`, onClick: () => this.toggleComponent() }, core$1.h("p", { class: "option-with-details-name dxp-text-truncate", title: this.value }, this.value, !this.toggle ? core$1.h("i", { class: `${this.iconSprite} arrow-down-xs-lo` }) : ''), core$1.h("p", { class: `option-with-details-location dxp-text-truncate ${this.toggle ? 'p-r-24' : ''}`, title: this.additionalValue }, this.additionalValue), this.toggle ? core$1.h("span", { class: "close-selector-menu" }, core$1.h("i", { class: "dxp-icon dxp-icon-close" })) : ''));
    }
    /** Render spinner */
    renderProgressbarSpinner() {
        ...
        return (core$1.h("div", { class: "spinner-wrapper" }, core$1.h("dxp-progressspinner", { "stroke-width": "1", fill: "rgba(255,255,255,0)", "animation-duration": "2", "stroke-color": "#25836D" })));
    }
    ...
    renderSearchableSelector() {
        ...
        return (core$1.h("div", { class: "selector-button-wrapper search-container", "aria-haspopup": "listbox", "aria-owns": this.selectorId, "aria-expanded": this.toggle }, core$1.h("input", { type: "text", class: `input-search dxp-text-truncate
          ${!this.isValid ? HAS_ERROR : ''} ${this.searchTerm.length > 0 || this.optionSelected ? 'more-padding' : ''}`, "aria-autocomplete": "list", "aria-controls": this.selectorId, "aria-invalid": !this.isValid, "aria-label": this.accessibilityText, "aria-required": !this.isOptional ? 'false' : 'true', autofocus: this.autofocus, disabled: this.disabled, id: this.selectorId, maxlength: this.maxSearchCharLength, placeholder: this.placeholder, tabindex: `${this.disabled ? -1 : 0}`, value: this.value ? this.value : this.searchTerm, onFocus: event => this.onSearch(event), onInput: event => this.onSearch(event) }), core$1.h("span", { class: `select-caret ${this.toggle ? ROTATE_ICON : ''} ${this.disabled ? DISABLED_CLASS : ''}` }, core$1.h("i", { class: `${this.iconSprite} caret-down-sm-b` })), core$1.h("button", { tabindex: `${this.disabled ? -1 : 0}`, role: "button", class: `clear-search-btn ${this.disabled ? DISABLED_CLASS : ''} ${this.searchTerm.length > 0 || this.value.length > 0 ? '' : 'dxp-none'}`, onClick: () => this.clearSearchBox(), "aria-label": "Clear searchbox" }, core$1.h("i", { class: "dxp-icon dxp-icon-close" })), !this.isValid ? core$1.h("div", { class: "dxp-error", id: "errMsg", "aria-label": this.validationMessage }, this.validationMessage) : ''));
    }
    ...
    setArrayValues(items) {
        return items.reduce((acc, cur, i) => {
            if (this.menuType === MenuType.LinkSelector && items.length === 1) {
                acc.push(core$1.h("p", { innerHTML: cur }));
            }
            else {
                if (i === 0) {
                    acc.push(core$1.h("p", { class: "ff-medium", innerHTML: cur }));
                }
                else {
                    acc.push(core$1.h("p", { innerHTML: cur }));
                }
    ...
    render() {
        core$1.dxp.log.debug(this.element.tagName, 'render()', `in dxp-selector-menu render() : ${"DEVELOPMENT"}`);
        const styles = [
            core$1.h("link", { rel: "stylesheet", href: `` }),
            [this.theme && core$1.h("link", { rel: "stylesheet", href: `` })],
            [this.theme && core$1.h("link", { rel: "stylesheet", href: `${core$1.dxp.config.get('DXP_STYLE_BASE_URL')}/themes/${this.theme}/dxp-selector-menu.min.css` })]
        ];
        return (core$1.h("div", { class: this.base.componentClass(), dir: this.dir, "data-theme": this.theme }, styles, this.label ?
            this.renderLabel()
            : '', core$1.h("div", { class: "selector-menu-wrapper" }, this.renderSelectorType(), core$1.h("div", { class: `item-container ${this.toggle ? 'dxp-block' : 'dxp-none'}` }, core$1.h("slot", { name: "item-container-slot" }), core$1.h("ul", { "aria-label": "Selector dropdown", class: `list-wrapper ${this.isLoading ? 'no-scroll' : ''}`, style: { width: this.width ? `${this.width}rem` : '100%' }, role: "listbox", "aria-labelledby": this.selectorId, onScroll: event => this.onListboxScroll(event) }, this.isLoading ? this.renderProgressbarSpinner() : '', core$1.h("slot", { name: "list-wrapper-slot" }), !this.disabled && this.items && this.items.length ?
            this.items.map((item, index) => core$1.h("li", { tabindex: "-1", class: "list-item", "data-index": index, id: `listItem-${index}`, role: "option", onClick: () => this.selectItem(item) }, this.renderItem(item)))
            : '')))));
    }
...
exports.dxp_selector_menu = SelectorMenu;
